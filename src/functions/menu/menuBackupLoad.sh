...
if [ $(echo ${isExist}) -eq 1 ]; then
	...
	tempResult="$(cat ${file} | grep --only-matching --perl-regexp "#${REPLY}\s${DIARY_BACKUPS_DIR}.*$")"
	tempResult="$(echo ${tempResult} | grep --only-matching --perl-regexp "${DIARY_BACKUPS_DIR}.*$")"
