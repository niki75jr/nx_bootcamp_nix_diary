...
function extractDates() {
	for file_eD in "$1"/*; do
		if [ -d "${file_eD}" ]; then
			...
			DATE="$(echo ${file_eD} | grep --only-matching --perl-regexp "/\w{8,8}__\d{4,4}-\d{2,2}")"
			DATE=${DATE:11:7}
...
function countPerDate () {
	...
	for i in ${!arrDATE[@]}; do
		...
		{ 
			echo -n "┌─────$curYEAR: "
			echo -n "$(cat ${file} | grep --perl-regexp "\w{8,8}__${curYEAR}-" | wc -l) "
			echo "records"
		}
		tempYEAR=${curYEAR}
		...
		echo -n "$(cat ${file} | grep --perl-regexp "\w{8,8}__${curYEAR}-${curMONTH}" | wc -l) "
		echo ;
	done
	fillChar ${#tempSTRING1} '█'; echo ''
}
...
if [ $(echo ${isExist}) -eq 1 ]; then
	...
	arrDATE=($(cat "${file1}" | sort --unique --reverse))
	countPerDate
else
