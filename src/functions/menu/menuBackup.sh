...
if [ $(echo ${isExist}) -eq 1 ]; then
	...
	for str in $(cat "${file}"); do
		...
		tempID="$(echo "${tempString1}" | grep --only-matching --perl-regexp "\`ID: \w{8,8}\`")"
		tempID=${tempID:5:8}
		tempDate="$(echo "${tempString1}" | grep --only-matching --perl-regexp "\`Date: \d{4,4}.\d{2,2}.\d{2,2}\`")"
		tempDate=${tempDate:7:10}
		tempTitle="$(echo "${tempString1}" | grep --only-matching --perl-regexp "#### \".*\"")"
